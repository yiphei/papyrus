...
export default function MapView() {
  ...
  useEffect(() => {
    ...
    // Phase 1 (sync): pre-register every category emoji, plus the emoji
    // placeholder under each asset-bearing event's icon id. Doing both
    // up-front closes a render-order race: the symbol layer recomputes
    // geojson synchronously during render with iconIds for new events,
    // but a per-event useEffect only runs after paint, so mapbox would
    // log "Image ... could not be loaded" and silently drop those pins.
    ...
    // Phase 2 (async): swap in real asset artwork for events with an
    // image_url. A failed fetch is caught and logged, leaving the emoji
    // placeholder in place — one bad URL no longer stalls the whole layer.
    for (const ev of events) {
